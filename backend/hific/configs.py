...
_CONFIGS = {
    "hific": helpers.Config(
        ...
        lambda_schedule=helpers.Config(vals=[2.0, 1.0], steps=[50000]),
        lr=1e-4,
        lr_schedule=helpers.Config(vals=[1.0, 0.1], steps=[500000]),
        num_steps_disc=1,
        loss_config=helpers.Config(
            ...
            target_schedule=helpers.Config(
                vals=[0.20 / 0.14, 1.0],  # Factor is independent of target.
                steps=[50000],
            ),
            lmbda_a=0.1 * 2.0**-6,
            lmbda_b=0.1 * 2.0**1,
...
def get_config(config_name):
    if config_name not in _CONFIGS:
        raise ValueError(f"Unknown config_name={config_name} not in {_CONFIGS.keys()}")
