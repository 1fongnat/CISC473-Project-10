...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path to the folder where checkpoints should be "
    ...
    parser.add_argument(
        ...
        default="1M",
        help=(
            "Number of steps to train for. Supports M and k "
            'postfix for "million" and "thousand", resp.'
        ),
    )
    parser.add_argument(
        "--init_autoencoder_from_ckpt_dir",
        metavar="AUTOENC_CKPT_DIR",
        help=(
            "If given, restore encoder, decoder, and "
    ...
    parser.add_argument(
        "--lpips_weight_path",
        help=("Where to store the LPIPS weights. Defaults to current directory"),
    )
    ...
    if args.ckpt_dir == args.init_autoencoder_from_ckpt_dir:
        raise ValueError(
            "--init_autoencoder_from_ckpt_dir should not point to "
