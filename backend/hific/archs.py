...
from compare_gan.architectures import abstract_arch, arch_ops
...
DiscOutAll = collections.namedtuple("DiscOutAll", ["d_all", "d_all_logits"])


# Split each tensor in a  DiscOutAll into 2.
DiscOutSplit = collections.namedtuple(
    "DiscOutSplit", ["d_real", "d_fake", "d_real_logits", "d_fake_logits"]
)
...
class Encoder(tf.keras.Sequential):
    ...
    def __init__(
        ...
        model.append(
            tf.keras.layers.Conv2D(
                filters=num_filters_bottleneck, kernel_size=3, padding="same"
            )
        )
...
class Decoder(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        head = [
            ChannelNorm(),
            tf.keras.layers.Conv2D(
                filters=num_filters_base * (2**num_up), kernel_size=3, padding="same"
            ),
            ChannelNorm(),
        ]
        ...
        # Final conv layer.
        tail.append(tf.keras.layers.Conv2D(filters=3, kernel_size=7, padding="same"))
...
class ResidualBlock(tf.keras.layers.Layer):
    ...
    def __init__(
        self, filters, kernel_size, name=None, activation="relu", **kwargs_conv2d
    ):
        """Instantiate layer.
...
class _PatchDiscriminatorCompareGANImpl(abstract_arch.AbstractDiscriminator):
    ...
    def apply(self, x):
        ...
        latent = arch_ops.conv2d(
            latent, 12, 3, 3, 1, 1, name="latent", use_sn=self._spectral_norm
        )
        latent = arch_ops.lrelu(latent, leak=0.2)
        latent = tf.image.resize(
            latent, [x_shape[1], x_shape[2]], tf.image.ResizeMethod.NEAREST_NEIGHBOR
        )
        ...
        num_filters = min(num_filters * 2, 512)
        net = arch_ops.conv2d(
            net, num_filters, k, k, 1, 1, name="d_conv_a", use_sn=self._spectral_norm
        )
        net = arch_ops.lrelu(net, leak=0.2)

        # Final 1x1 conv that maps to 1 Channel
        net = arch_ops.conv2d(
            net, 1, k, k, 1, 1, name="d_conv_b", use_sn=self._spectral_norm
        )
...
class _CompareGANLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, name, compare_gan_cls, **compare_gan_kwargs):
        """Constructor.
...
class Discriminator(_CompareGANLayer):
    def __init__(self):
        super(Discriminator, self).__init__(
            name="Discriminator", compare_gan_cls=_PatchDiscriminatorCompareGANImpl
        )
...
class Hyperprior(tf.keras.layers.Layer):
    ...
    def __init__(self, num_chan_bottleneck=220, num_filters=320, name="Hyperprior"):
...
class FactorizedPriorLayer(tf.keras.layers.Layer):
    ...
    def __init__(self):
        ...
        self._entropy_model = tfc.EntropyBottleneck(name="entropy_model")
