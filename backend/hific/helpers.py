...
def ensure_lpips_weights_exist(weight_path_out):
    ...
    if not os.path.isfile(weight_path_out):
        raise ValueError(
            f"Failed to download LPIPS weights from {_LPIPS_URL} "
...
def add_tfds_arguments(parser):
    parser.add_argument(
        "--tfds_dataset_name", default="clic", help="TFDS dataset name."
    )
    parser.add_argument(
        "--tfds_downloads_dir",
        default=None,
        help=("Where TFDS stores data.Defaults to ~/tensorflow_datasets."),
    )
    parser.add_argument(
        "--tfds_features_key", default="image", help="Name of the TFDS feature to use."
    )


def parse_tfds_arguments(args) -> TFDSArguments:
    print(f"SF: Using dataset {args.tfds_dataset_name}")
    return TFDSArguments(
        args.tfds_dataset_name, args.tfds_features_key, args.tfds_downloads_dir
    )
