...
import archs
import helpers
from helpers import ModelMode, ModelType, TFDSArguments
...
BppPair = collections.namedtuple("BppPair", ["total_nbpp", "total_qbpp"])
...
class _LossScaler:
    ...
    def get_rd_loss(self, distortion_loss, bpp_pair: BppPair, step):
        ...
        weighted_rate = self._get_weighted_rate_loss(loss_config, bpp_pair, step)
    ...
    def _get_weighted_rate_loss(self, loss_config, bpp_pair, step):
        ...
        lmbda_inv = tf.where(total_qbpp > target_bpp, 1 / lmbda_a, 1 / lmbda_b)
    ...
    def _get_scheduled_param(self, param, param_schedule, global_step, name):
        if (
            not self._ignore_schedules
            and param_schedule.vals
...
class HiFiC:
    ...
    def __init__(
        ...
        self._setup_discriminator = self._model_type == ModelType.COMPRESSION_GAN and (
            self.training or self.validation
        )  # No disc for evaluation.

        if self._setup_discriminator:
            self._num_steps_disc = self._config.num_steps_disc
            if self._num_steps_disc == 0:
                raise ValueError(
                    f"model_type=={self._model_type} but num_steps_disc == 0.",
                )
            tf.logging.info(
        ...
        self.input_spec = {
            "input_image": tf.keras.layers.InputSpec(
                dtype=tf.uint8,
    ...
    def build_input(
        ...
        dataset = self._get_dataset(batch_size, crop_size, images_glob, tfds_arguments)
        ...
        with tf.name_scope("tfds"):
            if images_glob:
                ...
                class SimpleTFDSBuilder:
                    ...
                    def as_dataset(self, split="train"):
                        ...
                        print(len(files))
                        raw_ds = tf.data.TFRecordDataset(files)
                        ...
                        # Just map normally (no AUTOTUNE)
                        ds = raw_ds.map(_parse, num_parallel_calls=4)
                        return ds
                ...
                print(dataset.reduce(0, lambda x, _: x + 1))

                # print("SF:", len([i for i in dataset]))
                quit()
                # tf.logging.info(f"Using TFDS={tfds_arguments}")
                # data_dir = "~/tensorflow_datasets/clic"
                # import tensorflow_datasets as tfds

                # from clic.clic import (
                #     Builder,  # or from your path: from my_project.datasets.clic import CLIC
                # )
                ...
                # # Prepare (will reuse if already downloaded)
                # builder.download_and_prepare(download_dir=data_dir)

                # # Load dataset
                # datasets = builder.as_dataset(split="train")

                # for example in datasets.take(1):
                #     print(example)
                # # dataset = builder.as_dataset(
                # #     split="train" if self.training else "validation"
                # # )
                # # Build the dataset object (this doesn't load data yet)
                # builder = tfds.builder("clic", data_dir=data_dir)

                # # Prepare (will reuse existing data if already downloaded)
                # builder.download_and_prepare(download_dir=data_dir)
                ...
                #     tfds_arguments.dataset_name,
                #     data_dir="~/tensorflow_datasets/clic",
                #     # tfds_arguments.downloads_dir
                # )
            ...
            def _preprocess(features):
                ...
                if not crop_size:
                    return image
                tf.logging.info(
                    "Scaling down %s and cropping to %d x %d",
                    image,
                    crop_size,
                    crop_size,
                )
                with tf.name_scope("random_scale"):
                    ...
                    image = tf.image.resize_images(
                        image,
                        [tf.ceil(scale * height), tf.ceil(scale * width)],
                    )
                with tf.name_scope("random_crop"):
                    image = tf.image.random_crop(image, [crop_size, crop_size, 3])
                return image

            dataset = dataset.map(_preprocess, num_parallel_calls=DATASET_NUM_PARALLEL)
    ...
    def build_model(self, input_image, input_images_d_steps=None):
        ...
        nodes_gen, bpp_pair, bitstrings = self._compute_compression_graph(input_image)

        if self.evaluation:
            tf.logging.info("Evaluation mode: build_model done.")
            reconstruction = tf.clip_by_value(nodes_gen.reconstruction, 0, 255.0)
        ...
        if self._setup_discriminator:
            ...
            disc_optimizer = self._make_discriminator_optimizer(self._global_step_disc)
    ...
    def _compute_compression_graph(self, input_image, create_summaries=True):
        ...
        with tf.name_scope("scale_down"):
            input_image_scaled = tf.cast(input_image, tf.float32) / 255.0
    ...
    def _get_encoder_out(self, input_image_scaled, image_shape) -> archs.HyperInfo:
        """Compute encoder transform."""
        encoder_out = self._encoder(input_image_scaled, training=self.training)
        return self._entropy_model(
            encoder_out,
    ...
    def _compute_reconstruction(self, decoder_in, image_shape, output_shape):
        ...
        """
        reconstruction_scaled = self._decoder(decoder_in, training=self.training)
        ...
        with tf.name_scope("re_scale"):
            reconstruction = reconstruction_scaled * 255.0
    ...
    def _compute_perceptual_loss(self, nodes: Nodes):
        ...
        return self._lpips_loss(reconstruction_scaled, input_image_scaled)

    def _create_gan_loss(
        self,
    ...
    def _train_generator(self, nodes: Nodes, bpp_pair: BppPair, step):
        ...
        with tf.name_scope("train_gen"):
            if self._setup_discriminator:
                d_outs = self._compute_discriminator_out(
                    nodes,
                    create_summaries=False,
                    gradients_to_generator=True,
                )
                g_loss = self._create_gan_loss(
                    d_outs,
                    create_summaries=True,
                    mode="g_loss",
                )
            ...
            if self._lpips_loss is not None:
                ...
                weighted_perceptual_loss = self._lpips_loss_weight * perceptual_loss
                tf.summary.scalar("weighted_lpips", weighted_perceptual_loss)
    ...
    def _get_and_check_variables(self):
        ...
        transform_vars = list(
            itertools.chain.from_iterable(
                x.trainable_variables for x in self._transform_layers
            ),
        )
        # Just getting these for book-keeping
        transform_vars_non_trainable = list(
            itertools.chain.from_iterable(x.variables for x in self._transform_layers),
        )
        disc_vars = (
            self._discriminator.trainable_variables if self._setup_discriminator else []
        )
        ...
        if (all_trainable != all_known) and all_trainable != set(
            transform_vars_non_trainable,
        ) | all_known:
            ...
            non_trainable_vars_str = "\n".join(
                sorted(v.name for v in transform_vars_non_trainable),
            )
            raise ValueError(
                "Did not capture all variables! "
                + " Missing in trainable: "
                + str(missing_in_trainable)
                + " Missing in known: "
                + str(missing_in_known)
    ...
    def _make_enc_dec_entropy_train_op(self, step, loss, entropy_vars, transform_vars):
        ...
        for i, (name, vs, l) in enumerate(
            ...
            minimize = optimizer.minimize(
                l,
                var_list=vs,
                global_step=step if i == 0 else None,
            )  # Only update step once.
...
class LPIPSLoss:
    ...
    def __init__(self, weight_path):
        ...
        self._lpips_func = tf.function(
            wrap_frozen_graph(graph_def, inputs=("0:0", "1:0"), outputs="Reshape_10:0"),
        )
