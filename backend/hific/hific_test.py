...
class FakeHiFiC(model.HiFiC):
    ...
    def _get_dataset(
        self, batch_size, crop_size, images_glob, tfds_arguments: helpers.TFDSArguments
    ):
        zeros = np.zeros((batch_size, crop_size, crop_size, 3))
        return (
            tf.data.Dataset.from_tensor_slices((zeros,)).repeat(128).batch(batch_size)
        )
...
class HiFiCTest(tf.test.TestCase):
    ...
    def test_input_pipeline(self):
        crop_size = 128
        hific = FakeHiFiC(configs.get_config("hific"), mode=helpers.ModelMode.TRAINING)
