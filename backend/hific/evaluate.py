    ...
    with tf.Session() as sess:
        ...
        for i in itertools.count(0):
            if max_images and i == max_images:
                break
            try:
                inp_np, otp_np, bitstring_np = sess.run(
                    [input_image, output_image, bitstring]
                )
                ...
                metrics = {"psnr": get_psnr(inp_np, otp_np), "bpp_real": bpp}

                metrics_str = " / ".join(
                    f"{metric}: {value:.5f}" for metric, value in metrics.items()
                )
                ...
                Image.fromarray(inp_np).save(os.path.join(out_dir, f"{name}_inp.png"))
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--ckpt_dir",
        required=True,
        help=("Path to the folder where checkpoints of the trained model are."),
    )
